...
func BuildPath(parts ...string) string {
	...
	if len(parts) < 1 {
		panic("Supply at least one path argument")
	}
	...
	for _, p := range parts {
		...
		path = strings.Trim(path, " /") + "/" + strings.Trim(p, " /")
	}

	if strings.Index(strings.Trim(parts[0], " "), "/") == 0 && strings.Index(path, "/") != 0 {
		return "/" + path
	}
...
func FileOpen(path string, mode int) (*os.File, error) {
	...
	return nil, errors.New("file does not exist. mode does not allow the creation of a new file.")
}
